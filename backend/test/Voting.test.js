...
describe("VOTING", async function () {
  ...
  describe("VOTER REGISTERING", async function () {
    ...
    it("The maximum proposal amount should not be more than 1000", async function () {
      await voting.addVoter(owner.address);
      await voting.startProposalsRegistering();
      for (let i = 0; i < 999; i++) {
        await voting.addProposal("description");
      }
      await expect(voting.addProposal("description")).to.be.revertedWith(
        "The maximum proposal amount is reached"
      );
    });
